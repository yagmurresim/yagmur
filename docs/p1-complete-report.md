# P1 düzeltme raporu — tam metin (güncel)

Migration'lar henüz hiçbir veritabanında çalıştırılmadı.
Uygulama sırası (fresh DB, signup kapalı, her dosya BEGIN/COMMIT):
...
`tsc --noEmit --incremental false` geçiyor.
...
| Madde | Düzeltme |
|---|---|
| 001 zayıf `auth.uid() IS NOT NULL` | baştan `is_admin()` |
| 001/002 atomik değil | her dosya BEGIN/COMMIT |
| Data API GRANT yok | 007 explicit matrix |
| `is_admin()` EXECUTE implicit | REVOKE ALL FROM PUBLIC; GRANT EXECUTE TO anon, authenticated |
| 007 sequence GRANT authenticated'a | yalnız service_role |
| 007 gelecekteki tablolar | runbook: her yeni tablo GRANT+RLS kendi migration'ında |
| Panel layout yalnız user | `requireAdminPage()` role gate |
| `updateSiteSettings` service-role | `createClient()` + RLS |
| SELECT hatası INSERT | maybeSingle + fail |
| 0 satır = başarı | `.select("id").maybeSingle()` + `requireRow` |
| maps_url boş string | preprocess → null |
| Form SEO 120/320 vs 70/160 | maxLength 70/160 |
| UUID/status/note/delete Zod yok | tüm write parse |
| lesson_formats serbest | individual \| group |
| yaş aralığı | Zod refine + DB CHECK |
| Storage public bucket | 006 private site-media |
| FAQ/KVKK preflight | 004 transaction başında |
| Singleton silinebilir | DELETE trigger |
| Admin login sidebar | (panel) route group |
| Lint kırık | eslint-config-next |
| apply.ts over-post | explicit projection; published program lookup; server-owned kvkk/status |

## Bilerek kapanmayan

1. 001–007 gerçek apply (SQL Editor / disposable Supabase) — deploy P0
2. Avukat onaylı KVKK / gizlilik — launch P0, commit P0 değil
3. Published media serving 200 — medya UI yoksa genel launch'ı bloklamaz

---

# B. src/server/auth.ts (tam)
...
# C. src/app/admin/(panel)/layout.tsx (tam)
...
# D. src/app/admin/layout.tsx (tam)
...
# F. src/lib/validation/schemas.ts (tam)
...
const TR_PHONE_REGEX = /^(\+90|0)?\s?[5][0-9]{2}\s?[0-9]{3}\s?[0-9]{2}\s?[0-9]{2}$/;
...
export const applicationSchema = z
  .object({
    ...
    student_age: z
      .number({ error: "Geçerli bir yaş girin." })
      .int("Tam sayı girin.")
      .min(4, "En düşük yaş 4'tür.")
      .max(100, "Geçerli bir yaş girin."),
    parent_name: z.string().max(120).optional().nullable(),
    phone: z
      .string()
      .regex(TR_PHONE_REGEX, "Geçerli bir Türkiye telefon numarası girin."),
    email: z
      .string()
      ...
      .or(z.literal(""))
      .nullable(),
    program_id: z.string().uuid("Lütfen bir eğitim seçin."),
    current_level: z.string().max(120).optional().nullable(),
    preferred_contact_channel: z
      .enum(["whatsapp", "phone"])
      .optional()
      .nullable(),
    preferred_contact_time: z.string().max(200).optional().nullable(),
    message: z.string().max(1000).optional().nullable(),
    kvkk_consent: z.literal(true, {
      message: "KVKK metnini onaylamanız gereklidir.",
    }),
    website: z.string().optional(), // honeypot
    _form_opened_at: z.number().optional(),
  })
  .superRefine((data, ctx) => {
    if (data.student_age < 18 && !data.parent_name?.trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "18 yaş altı öğrenciler için veli adı soyadı zorunludur.",
        path: ["parent_name"],
      });
    }
  });
...
export const contactSchema = z.object({
  ...
  phone: z
    .string()
    .regex(TR_PHONE_REGEX, "Geçerli bir telefon numarası girin.")
    .optional()
    .or(z.literal(""))
    .nullable(),
  email: z
...
# G. src/server/actions/apply.ts (tam)
...
import { applicationSchema, contactSchema, KVKK_VERSION } from "@/lib/validation/schemas";
import { checkApplicationRateLimit, getClientIP } from "@/lib/security/rate-limit";
import type { ApplicationInput, ContactInput } from "@/lib/validation/schemas";

const BOT_MIN_TIME_MS = 3000; // reject forms submitted faster than 3s
...
export async function submitApplication(
  rawData: ApplicationInput & { website?: string; _form_opened_at?: number }
): Promise<ApplyResult> {
  // Rate limiting
  const headersList = await headers();
  const ip = getClientIP({ headers: { get: (k: string) => headersList.get(k) } });
  const rl = await checkApplicationRateLimit(ip);

  if (!rl.success) {
    return {
      success: false,
      error: "Çok fazla başvuru gönderdiniz. Lütfen bir süre bekleyip tekrar deneyin.",
    };
  }
  ...
  if (rawData._form_opened_at) {
    const elapsed = Date.now() - rawData._form_opened_at;
  ...
  if (!parsed.success) {
    ...
    return { success: false, error: "Form bilgileri eksik veya hatalı.", fieldErrors };
  }
  ...
  if (siteUrl && referer && !referer.startsWith(siteUrl) && !referer.startsWith("http://localhost")) {
  ...
  try {
    ...
    if (programError) {
      console.error("[apply] program lookup:", programError.message);
      return {
        success: false,
        error: "Başvurunuz kaydedilemedi. Lütfen tekrar deneyin veya telefonla ulaşın.",
      };
    }
    if (!program) {
      return { success: false, error: "Seçilen eğitim bulunamadı." };
    }
    ...
    const { data: inserted, error } = await supabase
      ...
      .insert({
        ...
        current_level: data.current_level?.trim() || null,
    ...
    if (error || !inserted) {
      console.error("[apply] DB insert error:", error?.message ?? "no row");
      return {
        success: false,
        error: "Başvurunuz kaydedilemedi. Lütfen tekrar deneyin veya telefonla ulaşın.",
      };
    ...
    return {
      success: false,
      error: "Bir hata oluştu. Lütfen telefonla veya WhatsApp ile ulaşın.",
    };
  }
}
...
export async function submitContactMessage(
  rawData: ContactInput & { website?: string }
  ...
  try {
    ...
    if (error || !inserted) {
      console.error("[contact] DB insert error:", error?.message ?? "no row");
      return { success: false, error: "Mesajınız gönderilemedi." };
    }
...
# H. src/server/actions/admin.ts (tam)
...
# K. Admin profil (migration sonrası)
...
PostgREST RLS altında yetkisiz SELECT çoğu zaman permission denied üretmez; `[]` döner.

| Aktör | Beklenen |
|---|---|
| authenticated, profil yok | applications SELECT → `[]` |
| authenticated, profil yok | contact_messages SELECT → `[]` |
| authenticated, profil yok | draft programs UPDATE → 0 satır |
| admin profil | aynı satırlar görünür, UPDATE 1 satır etkiler |
| anon | unpublished site-media SELECT 400/deny |
| anon | published programs SELECT satır döner |

Medya özelliği kullanılabilir ilan edilmeden:
...
→ explicit kolon INSERT
```

Anon'a applications / contact_messages INSERT verilmedi.
Client payload `insert(formData)` olarak gitmez.
Server-owned: `kvkk_consent=true`, `kvkk_version=KVKK_VERSION`, `consented_at=now()`, `status=NEW|unread`.
`program_id` UUID + published lookup.
Rate limit + honeypot + min-time anti-bot duruyor.
